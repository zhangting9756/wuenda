...
def model_nihe(X,Y,learning_rate=0.3,num_iterations=30000,print_cost=True,is_plot=True,lambd=0,keep_prob=1):
    """
    实现一个三层的神经网络：LINEAR ->RELU -> LINEAR -> RELU -> LINEAR -> SIGMOID
...
if __name__ == '__main__':
    ...
    parameters = model_nihe(train_X, train_Y,is_plot=True)
    print("训练集:")
    ...
    """
    use zhengze
    parameters = model_nihe(train_X, train_Y, lambd=0.7,is_plot=True)
    print("使用正则化，训练集:")
    predictions_train = reg_utils.predict(train_X, train_Y, parameters)
    print("使用正则化，测试集:")
    predictions_test = reg_utils.predict(test_X, test_Y, parameters)
    ...
    reg_utils.plot_decision_boundary(lambda x: reg_utils.predict_dec(parameters, x.T), train_X, train_Y)
"""
    parameters = model_nihe(train_X, train_Y, keep_prob=0.86, learning_rate=0.3,is_plot=True)
